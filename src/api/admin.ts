import { createApiClient } from './chat';
...
export async function reloadFaqs(): Promise<string> {
  const response = await adminApiClient.post('/api/faq/reload');
  return response.data;
}

// FAQ 업데이트
export async function updateFaqs(): Promise<string> {
  const response = await adminApiClient.post('/api/faq/update');
  return response.data;
}

// FAQ 초기화 후 재로드
export async function resetAndReloadFaqs(): Promise<string> {
  const response = await adminApiClient.post('/api/faq/reset-and-reload');
  return response.data;
}

// FAQ 상태 확인
export async function getFaqStatus(): Promise<string> {
  const response = await adminApiClient.get('/api/faq/status');
  return response.data;
}
